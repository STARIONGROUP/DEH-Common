...
namespace DEHPCommon.UserInterfaces.ViewModels.Rows.ElementDefinitionTreeRows
{
    ...
    public abstract class ParameterValueBaseRowViewModel<T> : ParameterBaseBaseRowViewModel<T> where T : ParameterBase
    {
        ...
        protected ParameterValueBaseRowViewModel(T parameterBase, ISession session, ICDPMessageBus messageBus, IViewModelBase<Thing> containerViewModel) : base(parameterBase, session, messageBus, containerViewModel)
        {
            this.Initialize();
        }
